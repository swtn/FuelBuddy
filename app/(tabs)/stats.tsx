...
export default function StatsScreen() {
  ...
  const monthlyStats = useMemo(() => {
    const groups: {
      [key: string]: { count: number; totalCost: number; monthName: string };
    } = {};
    state.entries.forEach((entry) => {
      const date = new Date(entry.date);
      const key = `${date.getFullYear()}-${date.getMonth()}`;
      const monthName = date.toLocaleDateString("pl-PL", {
        month: "long",
        year: "numeric",
      });
  ...
  const chartData = useMemo(() => {
    ...
    const lastSixEntries = sorted.slice(-6);

    const labels = lastSixEntries.map((e) =>
      new Date(e.date).toLocaleDateString("pl-PL", {
        day: "2-digit",
        month: "2-digit",
      }),
    );
    const priceValues = lastSixEntries.map((e) => e.pricePerLiter);
    const literValues = lastSixEntries.map((e) => e.liters);
    ...
    return (
      <View style={styles.center}>
        <Text style={styles.emptyText}>
          Dodaj tankowania, aby zobaczyć wykresy
        </Text>
      </View>
    );
  }
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.chartBox}>
        ...
        <BarChart
          data={{
            labels: chartData.labels,
            datasets: [{ data: chartData.liters }],
          }}
          width={screenWidth - 70}
          height={190}
          yAxisLabel=""
          ...
          <View key={index} style={styles.monthlyCard}>
            <View>
              ...
              <Text style={styles.visitCount}>
                {stat.count} {stat.count === 1 ? "tankowanie" : "tankowania"}
              </Text>
            </View>
            <View style={styles.costBadge}>
              <Text style={styles.totalMonthCost}>
                {stat.totalCost.toFixed(2)} PLN
              </Text>
            </View>
          </View>
        ))}
